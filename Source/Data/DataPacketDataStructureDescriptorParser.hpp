...
WARP_TYPE_NAME_2(Data, DataPacketDataStructreDescriptorParser);

namespace Warp::Data {

	struct DataPacketDataStructreDescriptorInfo {
		std::unique_ptr<MVector<std::unique_ptr<DataPacketDataStructureDescriptor>>> _structs{};
		std::unique_ptr<MMap<MString, uint32_t>> _structs_name_to_idx{};
	};

	class DataPacketDataStructreDescriptorParser : public Inherit<DataPacketDataStructreDescriptorParser, Object> {
	...
	public:
		inline DataPacketDataStructreDescriptorParser() { _current_token.reserve(128); }

		inline void set_desc_text(std::string_view desc_text) { _desc_code = desc_text; }

		std::unique_ptr<DataPacketDataStructreDescriptorInfo> parse();
	...
	private:
		...
		void start_parse_single(DataPacketDataStructureDescriptor* struct_ptr);
	...
	private:
		...
		DataPacketDataStructreDescriptorInfo* _result{};
	};
};