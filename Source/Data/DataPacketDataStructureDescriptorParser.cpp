...
std::unique_ptr<DataPacketDataStructreDescriptorInfo> DataPacketDataStructreDescriptorParser::parse()
{
	...
	auto parser_result = std::make_unique<DataPacketDataStructreDescriptorInfo>();
	parser_result->_structs.reset(new MVector<std::unique_ptr<DataPacketDataStructureDescriptor>>());
...
void DataPacketDataStructreDescriptorParser::eat_space()
{
...
void DataPacketDataStructreDescriptorParser::get_next_token()
{
	_current_token = "";
...
bool DataPacketDataStructreDescriptorParser::start_parse_struct()
{
	...
	auto desc = std::make_unique<DataPacketDataStructureDescriptor>(struct_name, struct_desc, struct_alignment);
...
void DataPacketDataStructreDescriptorParser::start_parse_single(DataPacketDataStructureDescriptor* struct_ptr)
{
...
std::pair<DataPacketMemberType, MString> DataPacketDataStructreDescriptorParser::parse_typename()
{
	if (_current_token_type == DescTextTokenType::Identifer) {
...
MVector<uint32_t> DataPacketDataStructreDescriptorParser::parse_array_info()
{
	if (_current_token != "[") {
		put_token_back();
		return {};
...
MString DataPacketDataStructreDescriptorParser::parse_element_name()
{
	if (_current_token_type != DescTextTokenType::Identifer) {
		put_token_back();
		return "";
...
MString DataPacketDataStructreDescriptorParser::parse_desc_text()
{
	if (_current_token_type != DescTextTokenType::String) {
		put_token_back();
		return "";
...
uint16_t DataPacketDataStructreDescriptorParser::parse_alignment()
{
	if (_current_token_type != DescTextTokenType::Int) {
		put_token_back();
		return 1;
